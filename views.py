...
def build_image_url(element, customer_data, product_details, production_order_code, production_demand_code, pallet_number, box_number, kit_elements=None):
    ...
    order_clean = production_order_code.strip().upper()
    demand_clean = production_demand_code.strip().upper()
    customer_data = get_customer_data_for_element(element)
    build_folder_path_for_element(element)
...
def build_folder_path_for_element(element_data):
    """
    ...
    try:
        ...
        print(f"Extracted order_code: {order_code}, demand_code: {demand_code}")
...
# NOTE: everytime_check is now run automatically by the background scheduler
# The scheduler checks every 5 minutes for txt files in SOURCE_DIR
# See scheduler.py for configuration details
...
def scheduler_status(request):
    """
    Get the current status of the background file monitor scheduler
    """
    try:
...
def trigger_file_check_now(request):
    """
    Manually trigger the file check immediately
    (In addition to the automatic 5-minute interval check)
    """
    try:
...
def update_check_interval(request):
    """
    Update the interval (in minutes) for the background file check
    Default: 5 minutes
    
    POST parameter: interval (in minutes)
    """
    try:
